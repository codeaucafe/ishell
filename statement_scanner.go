...
type StreamScanner struct {
	...
	endedAtEOF bool
	// ignoreDelimiterStatements disables in-stream "DELIMITER x" detection. The
	// interactive shell sets this because it handles DELIMITER at the line level,
	// and the speculative lookahead read DELIMITER detection performs would block
	// a streaming reader on short complete statements.
	ignoreDelimiterStatements bool
}
...
func (s *StreamScanner) Scan() bool {
	...
	if !s.ignoreDelimiterStatements {
...
func (s *StreamScanner) read() error {
	...
	if s.fill == len(s.buf) {
		newBufSize := len(s.buf) * 2
		if newBufSize > s.maxSize {
			newBufSize = s.maxSize
		}
...
// StatementStartLine returns the 1-based line number on which the most recent
// token returned by Scan() began.
func (s *StreamScanner) StatementStartLine() int {
	return s.state.statementStartLine
}
...
// HasBufferedToken reports whether non-whitespace bytes remain buffered after
// the most recent token, i.e. another statement is already available without
// reading more input. Used by the interactive shell to decide whether a
// command is complete without triggering a blocking read.
func (s *StreamScanner) HasBufferedToken() bool {
	for j := s.i; j < s.fill; j++ {
		if !unicode.IsSpace(rune(s.buf[j])) {
			return true
		}
	}
	return false
...
// Delimiter returns the scanner's current statement delimiter, which may have
// changed from the initial value if a DELIMITER statement was scanned.
func (s *StreamScanner) Delimiter() string {
	return string(s.delimiter)
}

// IgnoreDelimiterStatements disables in-stream "DELIMITER x" detection. Callers
// that read from a blocking, line-at-a-time source (the interactive shell) set
// this so a short complete statement does not block on the DELIMITER lookahead;
// such callers handle DELIMITER changes themselves.
func (s *StreamScanner) IgnoreDelimiterStatements() {
	s.ignoreDelimiterStatements = true
}
